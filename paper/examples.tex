...
One feature of the mergeable rope implementation that is characteristic
...
\begin{itemize}
	...
	\item  \C{op\_transform} takes a pair of edit sequences, $s_1$ and $s_2$,
		...
		\C{op\_transform} (shown in the Appendix) computes such
...
as \C{Counter.merge} described in the introduction.  An alternative
instantiation of \C{MList} is for a quantity type
(\C{Qty}) that supports \C{add} and \C{subtract} operations, and
defines its \C{merge} operation a little differently:
...
to justify using ropes, but \C{MRope(MList(Item))} is certainly
allowed).  \C{MList(Item).merge} automatically lifts the merge
...
a single least common ancestor (LCA) for any pair of versions, the
