...
We now formalize the notion of mergeability, and the guarantees
...
$\under{H}{v_o \preceq v_o'}$. We write $L(v)$ to denote the locus
of version $v$.
\end{definition}

In a legal branching history, every version has a unique
locus\footnote{lemma and proof in the appendix}. Thus, locus of a
version is the maxima of its causal history. As usual, the locus of
the latest version on a branch is called the locus of the branch.
...
$\under{H}{v_2 \mbleto v_1}$) if and only if $L(v_1)$ is an ancestor
of $v_2$, and no internal ancestor of $v_1$ that is not also an
ancestor of $v_o$ is an ancestor of $v_2$. 
If $v_1$ and $v_2$ are the latest versions on their respective
branches $b_1$ and $b_2$, we say $b_2$ is mergeable into $b_1$, or
more generally, $b_1$ and $b_2$ are mergeable.  
\end{definition}

The following theorem asserts that every pair of branches deemed
mergeable by the above definition are infact mergeable, i.e., they are
guaranteed to have a unique LCA.
...
Thus, a pair of branches that are not currently mergeable (as per
Def.~\ref{fig:mergeability}), will become mergeable as the branching
history is extended.
