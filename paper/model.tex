...
In this section, we describe the \name programming model using a
collaborative drawing application we call \drawsome as a motivating
example.
...
expanded into tree structures lazily as and when pixels are colored.
The representation is thus optimized for sparse canvases, such as
...
and returning the pixel at a given coordinate. 
...
\C{v1} is white, and the same pixel in \C{v2} is green, and its color
in \C{lca} is white, then it means that only \C{v2} modified the
color. Hence the pixel is colored green in the merged canvas. On the
other hand, if the pixel is red in \C{v1}, then it means that both
\C{v1} and \C{v2} have modified the color. In such case, an
appropriate color-mixing algorithm can be used to determine the color
of pixel.  For instance, the pixel can be colored yellow - an additive
combination of red and green. The logic is illustrated in
Fig.~\ref{fig:canvas-merging}.  While the \drawsome\ merge operation
necessarily reflects domain-specific knowledge about the application,
it makes no reference to distribution or replication, and does not
impact the definition of any other operation, whose algorithms can be
developed for a sequential execution environment and seamlessly
transplanted to a distributed one.
\begin{figure}
\begin{center}
  \begin{ocaml}
...
computation.  This version is created by merging the proposal (the
local state) with a subset of concurrent versions that have become
available since the last merge or fork.  Thus, \C{sync} effectively
lets a computation sync with all computations operating over replicas
of the same state that are available (i.e., reachable in the
network)\footnote{The implementation implicitly triggers a sync
  whenever a detected network partition is repaired.}  to obtain their
latest updates.
\begin{figure}
\centering
\begin{tabular}{l||l||l}
...
concurrent version for Bob. Bob, in turn, invites Cheryl.
\begin{figure}
\centering
...
\end{figure}
All three start working on the blank canvas on different replicas.
...
in building concurrent applications with conceptual sharing of
state. This example, however, implicitly makes certain assumptions
...
theoretical challenges described in
Section~\ref{sec:semantics}. Unlike the canvas, the process of writing
merge functions for data types with non-trivial invariants could be
quite involved. The following section describes a principled approach
to deriving merge functions for any data type given its
invariants. Like \C{Canvas.merge}, merge functions often need to
compare multiple versions of a data type for structural equality,
which, when performed naively, incurs cost linear in the size of
versions. The representation of a versioned data type in \name is
optimized for performing comparisons that allows structural equality
to be determined in constant time. Such engineering details, which
contribute to the practicality of our programming model, are discussed
in Sec.~\ref{sec:impl}.
